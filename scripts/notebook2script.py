...
def notebook2scriptSingle(fname, output_name = 'test.py'):
    #print (fname)
    #print(list(Path("").iterdir()))
    #print(fname, output_name)
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(fname)
    fname_out = output_name
...
'''
    ...
    output_path = Path("../scripts")/fname_out; output_path.parent.mkdir(exist_ok = True)
    #if not (fname.parent.parent/'scripts').exists(): (fname.parent.parent/'scripts').mkdir()
    #output_path = fname.parent.parent/'scripts'/fname_out
    with io.open(output_path, "w", encoding="utf-8") as f:
        f.write(module[:-2])
    print(f"Converted {fname} to {output_path}")

if __name__ == '__main__': fire.Fire(notebook2scriptSingle)